...
IR::IfStatement *StatementGenerator::genConditionalStatement(bool is_in_func) {
    ...
    cond = target().expressionGenerator().genExpression(new IR::Type_Boolean());
    if (cond == nullptr) {
...
void StatementGenerator::removeLval(IR::Expression *left, IR::Type *type) {
...
IR::Statement *StatementGenerator::genAssignmentStatement() {
    ...
    switch (Utils::getRandInt(percent)) {
        case 0: {
            IR::Type_Bits *bitType = P4Scope::pickDeclaredBitType(true);
...
IR::Statement *StatementGenerator::genMethodCallExpression(IR::PathExpression *methodName,

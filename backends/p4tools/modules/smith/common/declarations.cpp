...
IR::Declaration_Constant *DeclarationGenerator::genConstantDeclaration() {
    ...
    IR::Type *tp = target().expressionGenerator().pickRndType(typePercent);
...
IR::Type_SerEnum *DeclarationGenerator::genSerEnumDeclaration(cstring name) {
    ...
    IR::Type_Bits *tp = ExpressionGenerator::genBitType(false);
...
IR::Method *DeclarationGenerator::genExternDeclaration() {
    ...
    IR::Type *returnType = target().expressionGenerator().pickRndType(typePercent);
...
IR::Type_Header *DeclarationGenerator::genEthernetHeaderType() {
    ...
    auto *ethDst = new IR::StructField("dst_addr", new IR::Type_Bits(48, false));
    auto *ethSrc = new IR::StructField("src_addr", new IR::Type_Bits(48, false));
    auto *ethType = new IR::StructField("eth_type", new IR::Type_Bits(16, false));
...
IR::Type_Header *DeclarationGenerator::genHeaderTypeDeclaration() {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        IR::Type *fieldTp = target().expressionGenerator().pickRndType(typePercent);

        if (auto *structTp = fieldTp->to<IR::Type_Struct>()) {
    ...
    if (P4Scope::req.byte_align_headers) {
        ...
        if (remainder != 0) {
            auto *padBit = new IR::Type_Bits(8 - remainder, false);
...
IR::Type *DeclarationGenerator::genHeaderStackType() {
    ...
    auto *ret = new IR::Type_Stack(hdrTypeName, new IR::Constant(new IR::Type_InfInt(), stackSize));
...
IR::Type *DeclarationGenerator::genType() {
